...
function detalleTarea(id) {
    ...
    if ($("#estado" + id).value == 1) {
        m = "<span class='border border-success text-success rounded p-2'>Tarea hecha!</span>";
    } else {
        m = "<span class='border border-danger text-danger rounded p-2'>Tarea por hacer!</span>"
    }
    $("#descripcion_m").innerHTML = $("#descripcion" + id).value;
    $("#estado_m").innerHTML = m;
    $("#fecha_m").innerHTML = getFecha($("#fecha" + id).value) + " " + getTime($("#hora" + id).value);
...
function habilitarEdicion(id) {
    ...
    selector.onkeyup = function() {
        document.querySelector("#show_num" + id).innerHTML = (100 - this.value.length) < 10 ? "0" + (100 - this.value.length) : (100 - this.value.length);
        if (this.value.length >= 100) {
            alert("Stop!");
        }
...
function getFecha(fecha) {
    ...
    var array = new Array("", "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
...
var ajax_req = function() {
    if (ajax.readyState == 4) {
        if (ajax.status == 200) {
            ...
            $("#numTareas").innerHTML = x[1] > 1 ? x[1] + " tareas restante para hoy de " + x[0] + "." : x[1] + " tarea restante para hoy de " + x[0] + ".";
        }
    }
