window.onload = function() {
    ...
    if (document.getElementById("getURL")) {
        var get = document.getElementById("getURL").value;
        if (get != "" && get == "A001") {
            $(document).ready(function() {
                $("#registro_user").html(
                    '<a href="?c=auth" class="btn btn-lg btn-primary btn-block text-uppercase">Iniciar Sesion</a>'
                );
            });
        }
    }
...
function enviar(event) {
    ...
    if (
        fecha[0] >= fecha_a.getFullYear() - 5 ||
        fecha[0] < fecha_a.getFullYear() - 120
    ) {
        ...
        if (clave1.length < 5) {
            showAlert(
                "warning",
                "Debes ingresar minimo 5 caracteres en la contraseña."
            );
            event.preventDefault();
        } else {
            if (clave1 != clave2) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.response === true) {
                                showAlert(
                                    "success",
                                    '<span class="text-peque">El usuario ha sido agregado correctamente.</span>'
                                );
                                $("#registro_user").html(
                                    '<a href="?c=auth" class="btn btn-lg btn-primary btn-block text-uppercase">Iniciar Sesion</a>'
                                );
                            } else if (data.response === "bad_email") {
                                showAlert("danger", "El correo que ingresaste ya esta en uso ");
                            } else if (data.response === "bad_nickname") {
                                showAlert(
                                    "danger",
...
if (document.getElementById("registro_user")) {
