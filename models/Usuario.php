...
class Usuario
{
    ...
    public function insert($data)
    {
        ...
        $stmt = $this->dbh->prepare("SELECT correo,nickname from Usuario ");
        $stmt->execute();
        ...
        try {
            ...
            if ($validacion == true) {
                ...
                $stmt = $this->dbh->prepare("SELECT id_usuario_PK from Usuario  Where correo = ?");
                $stmt->execute(array($data["correo"]));
                return array("response" => true, "idusuario" => $stmt->fetch()->id_usuario_PK);

            } else {
                return array("response" => $tipo);
            }
        } catch (Exception $e) {
            exit($e->getMessage());
        }
    ...
    public function getUserBy($idusuario, $valor)
    {
        try {
            $sql = "SELECT id_usuario_PK,nombre,apellido,nickname FROM Usuario WHERE id_usuario_pk != ? AND  (nombre like '%' ? '%' OR apellido like '%' ? '%' OR nickname like '%' ? '%') limit 5 ";
            $stmt = $this->dbh->prepare($sql);
            $stmt->execute(array($idusuario, $valor, $valor, $valor));
